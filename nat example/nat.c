...
unsigned int hook_func_in(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
{
	...
	//unsigned int dst_ip;
	//unsigned int path_id_incoming=0x7838A8C0; //192.168.56.120
	unsigned int ip_addr_incoming=0x6438A8C0; //192.168.56.100
	//Only listen on eth1
...
unsigned int hook_func_out(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
{
	...
	struct iphdr *ip_header;
	//unsigned int dst_ip;
    unsigned int path_id_outgoing=0x7938A8C0; //192.168.56.121
	//unsigned int ip_addr_outgoing=0x6538A8C0;//192.168.56.101
	//Only listen on eth1
