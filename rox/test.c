 ...
 */
	if(argc==9)
	{
		...
		inet_pton(AF_INET,argv[2],&(r.src_ip));
		inet_pton(AF_INET,argv[3],&(r.dst_ip));
		//r.src_ip=(unsigned int)strtol(argv[2], NULL, 10);
		//r.dst_ip=(unsigned int)strtol(argv[3], NULL, 10);
		r.protocol=(unsigned int)strtol(argv[4], NULL, 10);
		r.src_port=(unsigned int)strtol(argv[5], NULL, 10);
		r.dst_port=(unsigned int)strtol(argv[6], NULL, 10);

		inet_pton(AF_INET,argv[7],&(r.a.new_ip));
		//r.a.new_ip=(unsigned int)strtol(argv[7], NULL, 10);
		r.a.mark=(unsigned int)strtol(argv[8], NULL, 10);
